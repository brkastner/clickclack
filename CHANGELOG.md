...
- Added a full-origin navigation guard to the mobile Android project, since Capacitor compares only an app URL's scheme and host and would keep a same-host different-port navigation inside the privileged web view.
- Added GitHub sign-in inside the mobile app: the flow runs in the system browser with a PKCE challenge and hands a single-use grant back over a URL scheme, reusing the desktop client's existing server endpoints. Sign in with OpenClaw ID is hidden in the app until the server offers a native-client handoff for it, as it already is on desktop.
...
Fork integration retains multi-attachment sends: the first upload is atomic,
while `expected_attachment_count` declares the final total for consumers waiting
for the remaining uploads. Existing post-create attachment clients remain supported.
