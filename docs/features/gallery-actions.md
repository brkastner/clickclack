...
| `GET /api/gallery-actions/sessions/{session_id}/uploads/{upload_id}` | Initiating-actor-only image access for a choice or preview bound to a valid session. No message attachment is needed. Responses are private and non-cacheable. |
...
Source, choice thumbnails, and preview references must be authorized ClickClack upload IDs. Source previews use a native video element for video MIME types. Choice/response previews must be images in the same workspace. The installation bot can share its own uploads through the session-scoped image endpoint without posting messages. Other owners' uploads must already be accessible to both actor and bot. Session reads recheck expiry, capability generation, and source and destination access. The ordinary upload endpoint does not inherit session access. Arbitrary URLs, HTML, executable expressions, paths, or producer-selected destinations are unsupported.
