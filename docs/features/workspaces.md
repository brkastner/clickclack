...
it does not repeat a separate avatar beside the name. Viewers reorder groups by
dragging the header's move handle onto another profile, or by focusing the handle
...
replies exist. Existing DM thread routes keep their current behavior, but the
Copy link action is channel-only.
