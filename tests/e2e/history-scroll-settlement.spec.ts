...
async function historyFixture(request: APIRequestContext) {
  ...
  const alternateResponse = await request.post(`/api/workspaces/${workspace.id}/channels`, {
    data: { name: "restore-timeout" },
  });
  expect(alternateResponse.ok()).toBe(true);
  const { channel: alternateChannel } = await alternateResponse.json();
  const alternateBody = "Restore timeout destination";
  expect(
    (
      await request.post(`/api/channels/${alternateChannel.id}/messages`, {
        data: { body: alternateBody },
      })
    ).ok(),
  ).toBe(true);
  ...
  const otherUserID = clickclack([
    ...
    `History Bot ${suffix}`,
    "--email",
    `history-bot-${suffix}@example.com`,
  ]);
  const directResponse = await request.post("/api/dms", {
    data: { workspace_id: workspace.id, member_ids: [otherUserID] },
  });
  expect(directResponse.ok()).toBe(true);
  const { conversation } = await directResponse.json();
  const directPath = `/api/dms/${conversation.id}/messages`;
  for (let start = 0; start < 110; start += 10) {
    await Promise.all(
      Array.from({ length: 10 }, (_, index) =>
        request.post(directPath, {
          data: { body: `Attachment history ${start + index} ${"Grouped history. ".repeat(4)}` },
        }),
      ),
    );
  }
  const attachmentBody = "Attachment-heavy history destination";
  const heavyMessageResponse = await request.post(directPath, { data: { body: attachmentBody } });
  expect(heavyMessageResponse.ok()).toBe(true);
  const { message: heavyMessage } = await heavyMessageResponse.json();
  const png = Buffer.from(
    "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=",
    "base64",
  );
  for (let index = 0; index < 10; index += 1) {
    const uploadResponse = await request.post(`/api/uploads?workspace_id=${workspace.id}`, {
      multipart: {
        file: { name: `history-${index}.png`, mimeType: "image/png", buffer: png },
      },
    });
    expect(uploadResponse.ok()).toBe(true);
    const { upload } = await uploadResponse.json();
    const attachResponse = await request.post(`/api/messages/${heavyMessage.id}/attachments`, {
  ...
  return {
    ...
    alternateRoute: `/app/${workspace.route_id}/${alternateChannel.route_id}`,
    alternateBody,
    attachmentRoute: `/app/${workspace.route_id}/${conversation.route_id}`,
    attachmentBody,
...
test("a view restore fails open when animation frames stay paused", async ({ page }) => {
  const { route: appRoute, alternateRoute, alternateBody } = fixture;
  await page.goto(appRoute);
  await waitForAppReady(page);
  await settleScrollFrames(page);
  await pauseMessageFrames(page);
  try {
    await page.locator(`a[href="${alternateRoute}"]`).click();
    await expect(page).toHaveURL(alternateRoute);
    const messages = page.locator(".messages");
    await expect(messages).toHaveClass(/is-revealing/);
    await page.waitForTimeout(3_250);
    await expect(messages).not.toHaveClass(/is-revealing/);
    await expect(page.locator(".messages-scroll")).toHaveCSS("pointer-events", "auto");
  } finally {
    await page.evaluate(() => {
      const resume = Reflect.get(window, "resumeMessageFrames");
      if (typeof resume === "function") resume();
    });
  }
  await settleScrollFrames(page);
  await expect(page.getByText(alternateBody)).toBeVisible();
});

test("attachment-heavy direct history survives rapid navigation and late media sizing", async ({
  page,
}) => {
  const { route: appRoute, attachmentRoute, attachmentBody } = fixture;
  ...
  const attachmentLink = page
    .locator("#sidebar-direct-messages-list")
    .locator(`a[href="${attachmentRoute}"]`);
  try {
    await attachmentLink.click();
    await expect(page).toHaveURL(attachmentRoute);
    await mediaEntered.promise;
    await page.locator(`a[href="${appRoute}"]`).first().click();
  ...
  await expect(messages).not.toHaveClass(/is-revealing/);
  await expect(page.locator(".messages-scroll")).toHaveCSS("pointer-events", "auto");
  await expect(page.getByText(attachmentBody)).toBeAttached();
