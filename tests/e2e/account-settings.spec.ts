...
test("notification saves preserve profile changes from another tab", async ({ page, context }) => {
  ...
  await profile.getByLabel("Default or dark avatar URL").fill(avatar);
  await profile.getByLabel("Light mode avatar URL").fill(`${avatar}?light`);
  await profile.getByRole("button", { name: "Save profile" }).click();
  await expect(profile).toBeHidden();
