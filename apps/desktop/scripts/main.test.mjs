...
test("OAuth navigates only the application view and preserves the isolated terminal", async (t) => {
  const d = await desktop(t);
  const original = d.main;
  const [application, terminal] = original.views;
  const terminalLoads = [...terminal.webContents.loads];
  assert.equal(original.loadURL, undefined, "BaseWindow cannot navigate renderer content");
  d.send(original, "desktop:terminal-toggle");
  assert.ok(terminal.bounds.height > 0);
  await d.signIn();
  d.callback();
  await until(() => application.webContents.loads.length === 2);
  await settle();
  assert.deepEqual(application.webContents.loads, [`${A}/app`, `${A}/app`]);
  assert.deepEqual(terminal.webContents.loads, terminalLoads);
  assert.equal(terminal.webContents.isDestroyed(), false);
  assert.ok(terminal.bounds.height > 0);
  assert.deepEqual(d.errors, []);
});

test("server replacement disposes both sibling views and retires old application IPC", async (t) => {
  const d = await desktop(t);
  const original = d.main;
  d.send(original, "desktop:terminal-toggle");
  assert.ok(original.views[1].bounds.height > 0);
  d.send(original, "desktop:set-unread", 4);
  await d.save(B);
  assert.equal(original.destroyed, true);
  for (const view of original.views) assert.equal(view.webContents.isDestroyed(), true);
  assert.equal(d.main.views[1].bounds.height, 0);
  d.send(original, "desktop:set-unread", 99);
  assert.equal(d.badges.at(-1), 4);
  d.send(original, "desktop:terminal-toggle");
  assert.equal(d.main.views[1].bounds.height, 0);
  d.send(d.main, "desktop:set-unread", 5);
  assert.equal(d.badges.at(-1), 5);
  d.send(d.main, "desktop:terminal-toggle");
  assert.ok(d.main.views[1].bounds.height > 0);
});

test("OAuth completion is retired when its application contents are destroyed", async (t) => {
  const d = await desktop(t);
  const held = deferred();
  const original = d.main;
  d.controls.fetch = () => held.promise;
  await d.signIn();
  d.callback();
  await until(() => d.requests.length === 1);
  original.applicationContents.close();
  held.resolve(new Response("{}"));
  await settle();
  assert.equal(d.main, original);
  assert.deepEqual(original.loads, [`${A}/app`]);
  assert.equal(d.requests.length, 1, "A retired view cannot begin the session probe");
  assert.deepEqual(d.errors, []);
});
