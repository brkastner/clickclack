...
func testGetPasswordLoginIdentifierMatching(t *testing.T, st fixture) {
	...
	if _, err := st.UpdateUserProfile(ctx, store.UpdateUserProfileInput{
		...
		AvatarURLLight: "https://example.com/avatar-light.png",
	}); err != nil {
		t.Fatal(err)
	}
	...
	for _, identifier := range []string{"maggie@example.com", "MAGGIE@EXAMPLE.COM", "maggie", "Maggie"} {
		...
		if err != nil || login.User.ID != user.ID || login.User.AvatarURL != "https://example.com/avatar.png" || login.User.AvatarURLLight != "https://example.com/avatar-light.png" {
