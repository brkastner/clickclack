...
type GetAppearancePreferencesRow struct {
	...
	HidePersonaGrid      int64  `json:"hide_persona_grid"`
...
WHERE u.id IN (
  ...
  WHERE candidate.provider = ?1
     OR lower(candidate.email) IN (SELECT value FROM json_each(CAST(?2 AS TEXT)))
)
ORDER BY u.id, i.id
`
...
type ListThreadReplyPageParams struct {
	...
	DescendingOrder int64         `json:"descending_order"`
	PageLimit       int64         `json:"page_limit"`
}
