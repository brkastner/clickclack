...
func (q *Queries) ListMentionedUserIDs(ctx context.Context, arg ListMentionedUserIDsParams) ([]ListMentionedUserIDsRow, error) {
	rows, err := q.db.QueryContext(ctx, listMentionedUserIDs, arg.WorkspaceID, pq.Array(arg.Handles))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
type ListOutputMessagesParams struct {
	...
	WorkspaceID string `json:"workspace_id"`
	AuthorID    string `json:"author_id"`
	IncludeOwn  int32  `json:"include_own"`
	UserID      string `json:"user_id"`
	MediaOnly   int32  `json:"media_only"`
...
type UpsertBotRuntimeStatusParams struct {
	...
	FastMode             sql.NullBool `json:"fast_mode"`
	UpdatedAt            string       `json:"updated_at"`
	ExpiresAt            string       `json:"expires_at"`
}
