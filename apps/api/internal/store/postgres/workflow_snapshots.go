...
func (s *Store) ListWorkflowSnapshots(ctx context.Context, workspaceID, channelID, dmID, requesterID, cursor string, limit int) (store.WorkflowRunPage, error) {
	...
	rows, err := s.q.WithTx(tx).ListWorkflowSnapshots(ctx, storedb.ListWorkflowSnapshotsParams{WorkspaceID: workspaceID, ChannelID: channelID, DirectConversationID: dmID, CursorID: cursor, PageLimit: int32(limit + 1)})
	if err != nil {
		return page, err
	}
	if len(rows) > limit {
