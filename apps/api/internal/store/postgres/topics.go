...
func (s *Store) requireTopic(ctx context.Context, workspaceID, channelID, topicID string) error {
	...
	var topicWorkspace, topicChannel string
	if err := s.db.QueryRowContext(ctx, `SELECT workspace_id, COALESCE(channel_id, '') FROM topics WHERE id = $1 AND archived_at IS NULL`, topicID).Scan(&topicWorkspace, &topicChannel); err != nil {
