<script lang="ts">
  ...
  function startNotepadHover(channelID: string, anchor: HTMLElement) {
    ...
    const hoveredWorkspaceID = workspaceID;
    notepadHoverTimer = setTimeout(() => {
      if (workspaceID === hoveredWorkspaceID && anchor.isConnected) onNotepadHover?.(channelID, anchor);
    }, 120);
  }
...
{#snippet channelRow(channel: Channel, scope: Channel[], groupKey: string, subdued = false)}
  ...
  <div class="channel-row" class:subdued class:reorderable={expanded} role="listitem" class:drop-before={dropTargetID === channel.id && dropBefore} class:drop-after={dropTargetID === channel.id && !dropBefore}
    ...
    <a href={hrefForChannel(channel.id)} class="nav-item channel" class:active={channel.id === selectedChannelID && !selectedDirectID} class:has-unread={unread > 0 && channel.id !== selectedChannelID}
      ...
      onclick={(event) => { if (!shouldHandleClientNavigation(event)) return; event.preventDefault(); stopNotepadHover(); onSelectChannel(channel.id); }}>
...
{#snippet personaShelf()}
  <div class="sidebar-profile-groups">
    {#each botGroups as group (group.profile.bot_user_id)}
      ...
      {@const personaExpanded = hasChannels && (personaExpansion[group.profile.bot_user_id] ?? true)}
      ...
      <section class="channel-subgroup profile-channel-group" role="group" class:profile-drop-target={dropGroupKey === `bot:${group.profile.bot_user_id}`}
        ...
        >
          ...
          <button type="button" class="channel-subgroup-toggle profile-source-link" class:active={conversation?.id === selectedDirectID} class:has-unread={unread.total > 0}
            aria-expanded={hasChannels ? personaExpanded : undefined} aria-controls={hasChannels ? personaListID : undefined}
            aria-label={unread.total > 0 ? `${group.profile.display_name}, ${unread.total} unread` : group.profile.display_name}
            onclick={() => {
              if (hasChannels) onTogglePersona?.(group.profile.bot_user_id);
              else if (conversation) onSelectDirect(conversation.id);
              else onStartDirect(group.profile.bot_user_id);
            }}>
