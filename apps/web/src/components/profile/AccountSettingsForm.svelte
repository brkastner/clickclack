<script lang="ts">
  ...
  type Props = {
    ...
    payload: () => Partial<Pick<User, "display_name" | "handle" | "avatar_url" | "avatar_url_light" | "notification_settings">>;
    disabled?: boolean;
    children: Snippet;
  ...
  let { section, payload, children, onUserUpdated, onSaved, disabled = false }: Props = $props();
...
<form class="settings-form" onsubmit={(event) => { event.preventDefault(); void save(); }}>
  ...
  <footer class="settings-footer">
    ...
    {/if}
    <button type="submit" class="settings-button settings-button--primary" disabled={saving || disabled}>
