...
{#if !instant}<dialog bind:this={dialog} aria-label={action.descriptor.label} oncancel={(event)=>{event.preventDefault();onClose();}}>
 ...
 <div class="panel-body">
  ...
  {#if result?.session.preview_upload_id}<div class="source-preview"><span class="eyebrow">preview</span><img class="preview" src={apiURL(`/api/uploads/${encodeURIComponent(result.session.preview_upload_id)}`)} alt="Action preview" /></div>{/if}
 </div>
 <form onsubmit={(event)=>{event.preventDefault();void submit();}}>
  <fieldset disabled={busy||!!submission||state!=="accepted"}>
   {#each fields as field (field.id)}
    ...
    {:else}<section class="choice-field" aria-label={field.label}><div class="choice-heading"><div><h3>{field.label}</h3><p>Pick up to {field.max} reference{field.max===1?"":"s"}.</p></div>{#if field.dynamic&&field.choices.length<100}<button class="secondary-button" type="button" onclick={()=>void loadChoices(field.id,field.choices.length)}>{busy?"Loading…":field.choices.length?"Load more":"Load choices"}</button>{/if}</div>{#if field.choices.length}<div class="choices">{#each field.choices as choice (choice.id)}<label class:selected={Array.isArray(values[field.id])&&(values[field.id] as string[]).includes(choice.id)}><input type="checkbox" checked={Array.isArray(values[field.id])&&(values[field.id] as string[]).includes(choice.id)} onchange={(e)=>toggle(field.id,choice.id,e.currentTarget.checked)} />{#if choice.upload_id}<img src={apiURL(`/api/uploads/${encodeURIComponent(choice.upload_id)}`)} alt="" />{/if}<span>{choice.label}</span></label>{/each}</div>{:else}<p class="empty-choices">Load choices to pick reference images.</p>{/if}</section>{/if}
   {/each}
  </fieldset>
  ...
  <footer>
   ...
   {#if !busy && ((!submission && !!error) || (!!submission && (state==="accepted" || state==="failed")))}<button class="secondary-button" type="button" onclick={reset}>Start new panel</button>{/if}
