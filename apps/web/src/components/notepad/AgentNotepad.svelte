  ...
  <section id="agent-notepad-card" class="agent-notepad__card" aria-label="Agent notepad" aria-busy={snapshot.state === "loading"} tabindex="-1">
   ...
   <div class="notepad-body">
    {#if snapshot.state === "ready" && snapshot.card}
     ...
     {/if}
     <p class="notepad-updated">Updated <time datetime={new Date(snapshot.card.updatedAt).toISOString()}>{new Date(snapshot.card.updatedAt).toLocaleString()}</time></p>
    {:else}
...
<style>
 .agent-notepad { position: absolute; z-index: 3; top: 0; right: 18px; height: 20px; }
 ...
 .agent-notepad__card { position: absolute; right: 0; bottom: 28px; width: min(27rem, calc(100vw - 28px)); max-height: min(30rem, 58dvh); overflow: hidden; border: 1px solid var(--line-strong); border-radius: 10px; background: var(--panel); box-shadow: var(--shadow); color: var(--text); }
 ...
 .notepad-steps { display: grid; gap: 5px; margin: 12px 0; padding: 0; list-style: none; }
 .notepad-steps li { display: grid; grid-template-columns: 18px minmax(0, 1fr); align-items: start; gap: 7px; margin: 0; padding: 7px 8px; border: 1px solid transparent; border-radius: 6px; line-height: 1.4; }
 ...
 @media (max-width: 520px) { .agent-notepad { right: 10px; } .agent-notepad__read-only { display: none; } .agent-notepad__card { width: min(27rem, calc(100vw - 20px)); max-height: min(26rem, 52dvh); } .notepad-body { max-height: min(calc(26rem - 62px), calc(52dvh - 62px)); } }
