<script lang="ts">
  ...
  let autoLoadAttachmentIDs = $derived(recentAutoLoadAttachmentIDs([root, ...replies]));
...
<!-- svelte-ignore a11y_no_noninteractive_element_interactions (Tracks scroll intent from the viewport and its interactive descendants.) -->
<div
        ...
        <div class="attachment-grid compact" aria-label="Attachments">
          {#each root.attachments as attachment (attachment.id)}
            <MediaAttachment
              ...
              eager={autoLoadAttachmentIDs.has(attachment.id)}
