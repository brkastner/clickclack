...
describe("Home recent-message projection", () => {
  ...
  it("groups recent conversations by persona and rolls up unread and working state", () => {
    ...
    const items = buildHomeRecentItems(
      [
        ...
        {
          ...source("beta", [message("beta-new", "2026-09-17T11:00:00Z", "beta", personaA)]),
          unreadCount: 3,
        },
  ...
  it("uses the message author persona before a deterministic assigned fallback", () => {
    ...
    assert.equal(
      resolveHomePersona(
        { assignedBotIDs: [assignedB.id, assignedA.id] },
        humanMessage,
        new Map([
    ...
    assert.equal(
      resolveHomePersona(
        { assignedBotIDs: [assignedA.id] },
        message("bot", "2026-09-17T12:00:00Z", "done", author),
        new Map([
          [assignedA.id, assignedA],
          [author.id, author],
        ]),
  ...
  it("skips deleted rows and turns markdown into a bounded plain-text preview", () => {
    const deleted = {
      ...message("deleted", "2026-09-17T12:01:00Z"),
      deleted_at: "2026-09-17T12:02:00Z",
    };
