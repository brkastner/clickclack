...
export type ThreadReplyDelivery =
  | { type: "sending"; nonce: string }
  | { type: "sent"; nonce: string; message: Message }
  | { type: "failed"; nonce: string; error: unknown };
type ReplyDraft = {
  body: string;
  quote: Message | null;
  sending: boolean;
...
export class ThreadController {
  ...
  async send(
    ...
    onDelivery?: (delivery: ThreadReplyDelivery) => void,
  ): Promise<void> {
    ...
    onDelivery?.({ type: "sending", nonce: submission.nonce });
    try {
      ...
      onDelivery?.({ type: "sent", nonce: submission.nonce, message: data.message });
      ...
      onDelivery?.({ type: "failed", nonce: submission.nonce, error });
